...
class Domain_Classifier(nn.Module):
    def __init__(self, inplanes, domain_class):
        ...
        self.fullyConnected2 = nn.Sequential(nn.Linear(1000, 10),
                                            nn.BatchNorm1d(10, affine=affine_par)
                                            )
        self.fullyConnected3 = nn.Sequential(nn.Linear(10, domain_class))
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x, alpha):
        ...
        reverse_feature = ReverseLayer.apply(out, alpha)

        out = self.fullyConnected1(reverse_feature)
        out = self.relu(out)
...
class ResNet101(nn.Module):
    def __init__(self, block, layers, num_classes, phase):
        ...
        self.layer6 = self._make_domain_classifier(Domain_Classifier, 2048, 2)
        #self.layer7 = self._make_pred_layer_Domain(Classifier_Module_Domain, 2048, [6, 12, 18, 24], [6, 12, 18, 24], 2)
    ...
    def _make_domain_classifier(self, block, inplanes, domain_class):
        return block(inplanes, domain_class)
