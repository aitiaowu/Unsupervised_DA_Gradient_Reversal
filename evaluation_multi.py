...
palette = [0, 0, 0, 128, 0, 0, 0, 128, 0, 0, 0, 128]     ## three color channels for each catagory 
...
def colorize_mask(mask):
    ...
    new_mask = Image.fromarray(mask.astype(np.uint8)).convert('P')   ## .convert('P'): save as '.png' format.
                                                                     ## fromarray: transfrom from numpy to Image 
    new_mask.putpalette(palette)   ## add color 
    return new_mask

def fast_hist(a, b, n):
    k = (a>=0) & (a<n)
    return np.bincount( n*a[k].astype(int)+b[k], minlength=n**2 ).reshape(n, n)  ## .bincount: record the number of apparance of each value(from 0 - w+) in an array
...
def compute_mIoU( gt_dir, pred_dir, devkit_dir='', restore_from='' ):
    ...
    for ind in range(len(gt_imgs)):
        ...
        if len(label.flatten()) != len(pred.flatten()):    ## .flatten(): strech to 1-dimension array 
